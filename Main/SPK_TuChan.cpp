...
void cTuLuyen::LoadImageTuLuyen()
{
	...
	{
		char fileName[100];
		sprintf(fileName, "WZen\\RankTitle\\TuLuyen\\TuLuyen_%d.tga", i);
...
void cTuLuyen::ShowTuLuyenSPK(int index,int PosX, int PosY)
{
	...
	this->SPK_TULUYEN_LVL = gCustomRankUser.m_Data[index].m_rTuLuyen;
	//this->SPK_TULUYEN_LVL = 1;
...
void cTuLuyen::DrawWindowNangCapTuLuyen()
{
	...
	int LvTuLuyen = gCustomRankUser.m_Data[gObjUser.lpViewPlayer->aIndex].m_rTuLuyen;
	//int LvTuLuyen = gInterface.m_DanhHieu;
	...
	gToolKit.CloseButton(512, 78, SPK_TuChan_BtnClose);
	...
	if (LvTuLuyen < MAX_LEVEL_TU_LUYEN)
	{
		...
		CustomFont.Draw(CustomFont.FontBold, StartX + 160 + 30, YUpCapTuLuyen + 13, 0x1FD2FFFF, 0x0, CuaSoW - 140, 0, 1, gConfig.TSPK_TEXT_ENGINE[232], GetInfoLvTiepTheo->YCItemSL, this->ItemDanhHieu(GetInfoLvTiepTheo->YCItemID, GetInfoLvTiepTheo->YCItemLv));
		#else
