...
#define pDrawColorButton2			((void(__cdecl*)(DWORD, float, float, float, float, float, float, DWORD)) 0x00790F20)
...
#define pRenderGoldRect	((void(*)(float x,float y,float w, float h,int b))0x00429A40)
template<typename Type,class T>
inline Type GetResizeX(T in)
{
	return static_cast<Type>(static_cast<float>(in) / pWinWidthReal); // Screen Ratio X
}

template<typename Type, class T>
inline Type GetResizeY(T in)
{
	return static_cast<Type>(static_cast<float>(in) / pWinHeightReal); // Screen Ratio Y
}