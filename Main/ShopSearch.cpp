...
void PersonalShopEx::Draw()
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------------------
	//RenderBitmap(500001,StartX, gInterface.Data[EXBEXO_MOCNAP_MAIN].Y , MainWidth, MainHeight, 0, 0, 1, 1, 1, 1, 0.0);

	gInterface.DrawFormat(eGold, (int)StartX + 10, (int)StartY + 10, 210, 3, "TÌM KIẾM ĐỒ SHOP TREO");
	// ---
...
//	gInterface.DrawFormat(eGold, ContentX + 10, ContentY + 8, 210, 3, gCustomMessage.GetMessage(84));
	...
	gInterface.DrawFormat(eGold, ContentX + 97, ContentY + 329, 100, 1, "Trang: %d/%d", this->Page + 1, (this->TotalShop - 1) / 10 + 1);
	...
	if (this->CheckWindowEx2(ePT_PERSONAL_SEARCH_MAIN))
	{
		for (int i = 0; i < 10; i++)
		{
			if (g_PersonalShopEx.ButtonEx(Event, ePT_PERSONAL_SEARCH_PAGEUP1 + i, false))
			{
				...
				pReq.header.set(0xD3, 0x20, sizeof(pReq));
...
void PersonalShopEx::CG_Send()
{
	...
	pRequest.h.set(0xD3, 0x22, sizeof(pRequest));
...
void PSShopVieweFix()
{
	...
	pReq.h.set(0xD3, 0x23, sizeof(pReq));
