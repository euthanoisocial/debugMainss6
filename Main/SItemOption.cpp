...
void ItemTooltipRender(int x, int y, int nCount, int witd, int height,int type, int Align)
{
	...
	int bonusLineHeight = 10;

	if (GItemIndex >= 3584 && GItemIndex < 6144) 
	{ //quan ao
		bonusLineHeight += 30;
	}
	...
	cRenderH = ( (gPostInterface.item_post_ && pCursorX < 180) ? ( SellItem + ( cRenderH / ( pWinHeightReal / v71 ))) : ( cRenderH / ( pWinHeightReal / v71 ))) + bonusLineHeight;
	...
	int itemLevel = 0;
	//-- Escritura
	int start = 0; //dong ten item
	if (gInterface.CheckWindow(Shop) || gInterface.CheckWindow(Store) || gInterface.CheckWindow(OtherStore))
		start = 2;
	...
	for( int k = 0; k < nCount; ++k )
	{
		if (k == start) {
			char name[60];
			strcpy(name, TooltipLine[k]);
			char* Line = strtok(name, "+");
			Line = strtok(NULL, "+");
			if (Line != NULL)
			{
				itemLevel = gCItemSetOption.parseInt(Line);
			}
		}
		...
		if (k == (start +1)) {
			...
			glColor3f(1, 0, 0);
			char strLevel[30];
			wsprintf(strLevel, "Item Cấp %d", level);
			pDrawText(pTextThis(), (double)(cRenderX + 1), posY - 10, strLevel, (cRenderW - 1.0f), 0, (LPINT)height, 0);
		}
		//gInterface.DrawMessage(0, "Camera: %d", lItem->ExcellentOption);

		if ((GItemIndex >= 3584 && GItemIndex < 6144) && gCItemSetOption.Contains(TooltipLine[k], "Dùng cho") && !isBuff && k >= start) { //dung cho ...
			isBuff = true;

			char buff[100];
			if(itemLevel >= 9 && itemLevel < 11)
				glColor3f(1, 0.2, 1);
			else
				glColor3f(0.40000001f, 0.40000001f, 0.40000001f); //gray

			wsprintf(buff, "Full set cấp 9 + 5%% sức mạnh"); //5%

			pDrawText(pTextThis(), (double)(cRenderX + 1), posY, buff, (cRenderW - 1.0f), 0, (LPINT)height, 0);
			v76 += 10;
			posY += 10;

			if (itemLevel >= 11 && itemLevel < 13)
				glColor3f(1, 0.2, 1);
			else
				glColor3f(0.40000001f, 0.40000001f, 0.40000001f); //gray

			wsprintf(buff, "Full set cấp 11 + 5%% sức mạnh"); //5%
			...
			wsprintf(buff, "Full set cấp 13 + 5%% sức mạnh"); //5%
		...
		if( ((DWORD) TooltipLine[k] != 10 && ( DWORD) TooltipLine[k] != 32 || (DWORD) TooltipLine[k] ) && lstrlenA(TooltipLine[k]) > 2 )
		{
			gCItemSetOption.RenderColor( ColorTexto[k] );
				...
				pDrawText( pTextThis(), (double)(cRenderX + 1), posY, TooltipLine[k], ( cRenderW - 1.0f ), 0, (LPINT)height, 0 );
...
bool CSItemOption::Contains(char MyChar[], const char* Search)
{
	int start = 0;
	int startSearch = 0;
	int sourceLength = strlen(MyChar);
	int SearchSize = strlen(Search);
	//int isOk = 0;
	int firstCharIndex = 0;
	while (start < sourceLength)
	{
		if (MyChar[start] == Search[startSearch])
		{
			if (startSearch == 0)
		...
		if (startSearch == SearchSize) {
			if (firstCharIndex == 0 && (MyChar[firstCharIndex + SearchSize] == ' ' || (firstCharIndex + SearchSize) == sourceLength))
				return true;
			else if (firstCharIndex > 0 && MyChar[firstCharIndex - 1] == ' ') {
				if ((firstCharIndex + SearchSize) == sourceLength)
					return true;
				else if (MyChar[firstCharIndex + SearchSize] == ' ')
					return true;
			}
		}
		start++;
