...
bool cThueFlag::CheckRect()
{
	...
	return gInterface.Data[SPK_ThueFlag_Main].OnShow = true && pCheckMouseOver(x, y, w, h);
}

void cThueFlag::Bind()
{
	gInterface.BindObject(SPK_ThueFlag_Main, 0x7A5A, 445, 364, 205, -1);
}
...
{
	if ((GetTickCount() - gInterface.Data[SPK_ThueFlag_Main].EventTick) > 300)
	{
		gInterface.Data[SPK_ThueFlag_Main].EventTick = GetTickCount();

		if (gInterface.Data[SPK_ThueFlag_Main].OnShow)
		{
			ThueFlag.SendRequest(0x2, 0);
			gInterface.Data[SPK_ThueFlag_Main].OnShow = 0;
			return;
		}
		ThueFlag.SendRequest(0x0, ThueFlag.Selected);
		gInterface.Data[SPK_ThueFlag_Main].OnShow ^= 1;
	}
...
void cThueFlag::Draw()
{
	...
	gCentral.gDrawWindowCustom(170, 70, 296, 260, SPK_ThueFlag_Main, "THUÊ FLAG");
	gToolKit.CloseButton(433, 69, SPK_ThueFlag_BtnClose);
	...
	CustomFont.Draw(CustomFont.FontNormal, x + 78, (y + 240) + 8.5, 0xFFFFFFFF, 0x0, 73, 0, 3, (DisableFlag == 1) ? gConfig.TSPK_TEXT_ENGINE[287] :gConfig.TSPK_TEXT_ENGINE[288]);
	...
	CustomFont.Draw(CustomFont.FontNormal, x + 218, (y + 240) + 8.5, 0xFFFFFFFF, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[289]);
	
	if (this->Selected > 0)
	{
		this->DrawButton(32331, x + 288, y + 240, 73, 27, 128.0, 128.0, ThueFlag.PrevAction);
		CustomFont.Draw(CustomFont.FontNormal, x + 288, (y + 240) + 8.5, 0xFFFFFFFF, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[290]);
	}

	if (skin != 0)
	{
		CustomFont.Draw(CustomFont.FontNormal, x + 95, (y + 0) + 65, 0x66FF66FF, 0x0, 150, 0, 1,  gConfig.TSPK_TEXT_ENGINE[291], skin->Value1);
		CustomFont.Draw(CustomFont.FontNormal, x + 95, (y + 0) + 80, 0x66FF66FF, 0x0, 150, 0, 1,  gConfig.TSPK_TEXT_ENGINE[292], skin->Value2);
		CustomFont.Draw(CustomFont.FontNormal, x + 95, (y + 0) + 95, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[293], skin->Value3);
		CustomFont.Draw(CustomFont.FontNormal, x + 95, (y + 0) + 110, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[294], skin->Value4);
		CustomFont.Draw(CustomFont.FontNormal, x + 95, (y + 0) + 125, 0x66FF66FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[295], (skin->Time) / 60);
		...
		//tue add debug fix
		BBDrawItem3D(x + 135 + (25 * 4), y + 15, 50, 60,
			//gProtect.m_MainInfo.m_mModelFlagData[skin->Id].ModelIndex, 0, 0, 0, 0, 0.8f, true);
			//1172, 0, 0, 0, 0, 0.8f, true);
			7780, 0, 0, 0, 0, 0.8f, true);
		////
		if (skin->Time < 1)
		{
			CustomFont.Draw(CustomFont.FontNormal, x + 65, y + 160, 0xFF3300FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[296], gToolKit.NumberFormat(skin->Price));
		}
		else
		{
			CustomFont.Draw(CustomFont.FontNormal, (x + 65) + 30, y + 160, 0xFF3300FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[296], gToolKit.NumberFormat(skin->Price));
		}
		CustomFont.Draw(CustomFont.FontNormal, (x + 65) + 30, (y + 160) + 15, 0xFF3300FF, 0x0, 150, 0, 1, gConfig.TSPK_TEXT_ENGINE[297], skin->Value7);
		...
		if (skin->Value10 != -1)
		{
			if (skin->Value10 > 0)
			{
				...
				CustomFont.Draw(CustomFont.FontNormal, x + 180, y + 60, 0xFFFFFFFF, 0x00FBFF69, 171, 0, 3, gConfig.TSPK_TEXT_ENGINE[298], strdup(dateTimeString.c_str()));
			}
			else
			{
				CustomFont.Draw(CustomFont.FontNormal, x + 4 + 75 * 2, y + 35, 0xffffffff, 0x0, 150, 0, 3, gConfig.TSPK_TEXT_ENGINE[299]);
			}
			CustomFont.Draw(CustomFont.FontNormal, x + 148, (y + 240) + 8.5, 0xffffffff, 0x0, 73, 0, 3, skin->Skin != this->Skin ? gConfig.TSPK_TEXT_ENGINE[300] : gConfig.TSPK_TEXT_ENGINE[301]);
		}
		else
		{
			CustomFont.Draw(CustomFont.FontNormal, x + 180, y + 60, 0xFFFFFFFF, 0x00FBFF69, 171, 0, 3, gConfig.TSPK_TEXT_ENGINE[302]);
			CustomFont.Draw(CustomFont.FontNormal, x + 148, (y + 240) + 8.5, 0xffffffff, 0x0, 73, 0, 3, gConfig.TSPK_TEXT_ENGINE[303]);
		}
