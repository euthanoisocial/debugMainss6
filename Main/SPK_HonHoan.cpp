...
{
    std::stringstream ss;
    ss << "WZen\\RankTitle\\HonHoan\\" << index << ".jpg";
    //ss << "WZen\\RankTitle\\HonHoan\\" << index << ".tga";
    return ss.str();
}
...
void cHonHoan::AddHonHoanCape(DWORD ObjectPointer)
{
	...
	if (gCustomRankUser.m_Data[Object->aIndex].m_rNguHanh > 0 && gCustomRankUser.m_Data[Object->aIndex].m_rNguHanh < LevelHH + 1)
	{
		vec3_t Color;
		Vector(1.0, 1.0, 1.0, Color);
		int capDoHonHoan = gCustomRankUser.m_Data[Object->aIndex].m_rNguHanh;
...
void cHonHoan::DrawWindowNangCapHonHoan()
{
	...
	int LvHonHoan = gCustomRankUser.m_Data[gObjUser.lpViewPlayer->aIndex].m_rNguHanh;
	...
	gToolKit.CloseButton(512, 78, SPK_HonHoan_BtnClose);
	...
	if (LvHonHoan < LevelHH) // so luong hon hoan
	{
		...
		CustomFont.Draw(CustomFont.FontBold, StartX + 160 + 30, YUpCapHonHoan + 13, 0x1FD2FFFF, 0x0, CuaSoW - 140, 0, 1, gConfig.TSPK_TEXT_ENGINE[232], GetInfoLvTiepTheo->YCItemSL, this->ItemDanhHieu(GetInfoLvTiepTheo->YCItemID, GetInfoLvTiepTheo->YCItemLv));
		#else
