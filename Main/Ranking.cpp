...
{
	if (GetTickCount() < gInterface.Data[SPK_Ranking_Main].EventTick + 300) return;
	gInterface.Data[SPK_Ranking_Main].EventTick = GetTickCount();
...
{
	LoadBitmapA("WZen\\MacroUI\\Btn_A_RankAll.tga",	0x17928, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_A_RankCoin.tga", 0x17929, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_A_Event.tga", 0x1792A, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_A_Guild.tga", 0x1792B, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_A_PkCount.tga", 0x1792C, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_A_Mter.tga", 0x1792D, GL_LINEAR, 0x2901, 1, 0);
	LoadBitmapA("WZen\\MacroUI\\Btn_B_PkCount.tga", 0x1792E, GL_LINEAR, 0x2901, 1, 0);
...
void DrawPlayerRank(int X, int Y, int W, int H)
{
	...
	{
		gInterface.DrawFormat(eGold, X + (W / 2), Y + (H / 2), 52, 1, "Đang tải...");
	}
	else
	{
		...
		{
			if (pIsKeyPress(VK_LBUTTON) && GetTickCount() > gInterface.Data[SPK_Ranking_Main].EventTick + 300)
			{
...
void cRanking::Draw()
{
	...
	gToolKit.CloseButton(475, 39, SPK_Ranking_BtnClose);
	pSetCursorFocus = true;
	char ClassName[11] = { 0 };
	...
	gToolKit.DrawButtonIconRank(ChonTopGuild,	0x1792B, StartX + 185, StartY + 30, 16, 19, "Top Guild");
	gToolKit.DrawButtonIconRank(ChonTopPK,		0x1792C, StartX + 200, StartY + 30, 16, 19, "Top PK");
	gToolKit.DrawButtonIconRank(ChonTopMT,		0x1792D, StartX + 215, StartY + 30, 16, 19, "Top MT");
	...
	switch (TrangRanking)
	{
		...
		if (ListScrollBarCase1 == NULL)
		{
			...
			ListScrollBarCase1->Create(PosXInvetoryImg + 210, PosYInvetoryImg + 40, 270);
			if (ListScrollBarCase1)
			{
				ListScrollBarCase1->SetMaxPos((this->DataList[0] <= this->ListMaxPer[0]) ? 0 : (this->DataList[0] - this->ListMaxPer[0]));
				ListScrollBarCase1->SetPos(PosXInvetoryImg + 210, PosYInvetoryImg + 40);
			}
		}
		if (ListScrollBarCase1)
		{
			...
			ListScrollBarCase1->MouseWheelWindow = pCheckMouseOver(PosXInvetoryImg - 113, PosYInvetoryImg + 40, 330, 270);
		...
		for (int i = this->CurPos[0]; i < iMaxCount; i++)
		{
			...
			if (pCheckMouseOver(StartX + 151, PosY + 75 + (this->Count[0] * MAX_LINE), (310), 13) == 1 && !ViewCharItems.getState())
			{
				if (strcmp(ClassName, "SPK") == 0)
				{
					...
					#endif
					if (pIsKeyPress(VK_LBUTTON) && GetTickCount() > gInterface.Data[SPK_Ranking_Main].EventTick + 100)
					{
						GetInfoCharTop(this->RankingChar[up].Name);
						gInterface.Data[SPK_Ranking_Main].EventTick = GetTickCount();
					}
		...
		gInterface.DrawFormat(eWhite, StartX + 175, StartY + 62, 100, 3, "WCOINC");
		gInterface.DrawFormat(eWhite, StartX + 225, StartY + 62, 100, 3, "WCOINP");
		gInterface.DrawFormat(eWhite, StartX + 275, StartY + 62, 100, 3, "WCOING");
		gInterface.DrawFormat(eWhite, StartX + 325, StartY + 62, 100, 3, "WCOINR");
		...
		if (ListScrollBarCase2 == NULL)
		{
			...
			ListScrollBarCase2->Create(PosXInvetoryImg + 210, PosYInvetoryImg + 40, 270);
			if (ListScrollBarCase2)
			{
				ListScrollBarCase2->SetMaxPos((this->DataList[1] <= this->ListMaxPer[1]) ? 0 : (this->DataList[1] - this->ListMaxPer[1]));
				ListScrollBarCase2->SetPos(PosXInvetoryImg + 210, PosYInvetoryImg + 40);
			}
		}
		if (ListScrollBarCase2)
		{
			...
			ListScrollBarCase2->MouseWheelWindow = pCheckMouseOver(PosXInvetoryImg - 113, PosYInvetoryImg + 40, 330, 270);
		...
		for (int i = this->CurPos[1]; i < iMaxCount; i++)
		{
			...
			if (i >= 0 && i <= 2){ Color = eGold; } else { Color = eWhite; }
		...
		gInterface.DrawFormat(eWhite, StartX + 105, StartY + 62, 100, 3, "TOP");
		gInterface.DrawFormat(eWhite, StartX + 130, StartY + 62, 100, 3, "NAME");
		gInterface.DrawFormat(eWhite, StartX + 175, StartY + 62, 100, 3, "KILL BOSS");
