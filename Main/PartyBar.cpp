...
char PrintfPartyBar(int a1)
{
	...
	if (*(BYTE *)(v43 + 917))
	{
		...
		for (i = 0; i < QTPary.PartyMemberCount; ++i)
		{
			...
			pDrawBarForm(x + 2, y -13, 74.0, 17.0, 0.0, 0);

			EnableAlphaTest(1);
			if (QTPary.JCPartyDB[i].ViewportID == -1)
			{
				glColor4f(0.30000001, 0.0, 0.0, 0.5);
				pDrawBarForm(x, y - 15, 74.0, 17.0, 0.0, 0);
				EnableAlphaTest(1);
			}
			else
			{
			...
			pDrawGUI(31557, x, y-15, 77.0, 23.0);
			...
			if (i)
			{
				...
				pDrawText(pTextThis(), (int)x + 4, (int)y -11, QTPary.JCPartyDB[i].Name, *(DWORD *)(v43 + 932), 0, (LPINT)1, 0);
			}
			else
			{
				...
				pDrawGUI(31553, x + 53, y -12, 9.0, 10.0);

				pDrawText(pTextThis(), (int)x + 4, (int)y -11, QTPary.JCPartyDB[0].Name, *(DWORD *)(v43 + 932), 0, (LPINT)1, 0);
			}
			...
			pDrawGUI(31558, x + 4, y +1, v39, 3.0);

			if (!strcmp(QTPary.JCPartyDB[0].Name, (char *)(*(DWORD *)0x7BC4F04 + 56))
				|| !strcmp(QTPary.JCPartyDB[i].Name, (char *)(*(DWORD *)0x7BC4F04 + 56)))
			{
				if (pCheckMouseOver(x + 63, y -12, 11, 11) && *(BYTE*)0x8793386)
				{
					pDrawButton(31554, x + 63, y -12, 11.0, 11.0, 0.0, 11.0);
				}
				else
				{
					pDrawButton(31554, x + 63, y -12, 11.0, 11.0, 0.0, 0.0);
				}
			}
			//Vẽ icon buff trên thanh bar
			gPartyBuffs.DrawPartyBuffs((double)x-8, (double)y-15, QTPary.JCPartyDB[i].Name);
		}
		pGLSwitch();
		result = 1;
	}
